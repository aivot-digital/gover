...
if [ "$1" = "serve" ]; then
  ...
  echo "Waiting for IDP to be available at ${GOVER_KEYCLOAK_OIDC_INTERNAL_HOSTNAME:-${GOVER_KEYCLOAK_OIDC_HOSTNAME}}/realms/staff…"

  until curl --output /dev/null --silent --head --fail "${GOVER_KEYCLOAK_OIDC_INTERNAL_HOSTNAME:-${GOVER_KEYCLOAK_OIDC_HOSTNAME}}/realms/staff/"; do
      sleep 5
  done
