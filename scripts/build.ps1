...
$version = Read-Host "Insert new version"

$confirm = Read-Host "Preparing version $version. Is this correct [y/n]?"
...
Write-Host "Preparing version $version ..."

Write-Host "Updating ./pom.xml"
sed -i '' -E "13s|.*|    <version>$version</version>|" ./pom.xml

Write-Host "Updating ./Dockerfile"
...
Write-Host "Updating ./app/package.json"
...
echo "All done! Version $version finished."
