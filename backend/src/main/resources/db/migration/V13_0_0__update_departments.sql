...
BEGIN
    -- 1. If parent_department_id is null, it's a top-level node.
    ...
    v_current_id := NEW.parent_department_id;
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Parent org unit ID % does not exist.', NEW.parent_department_id;
    END IF;
