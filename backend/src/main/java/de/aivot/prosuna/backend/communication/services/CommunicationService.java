...
public class CommunicationService {
    ...
    public CustomerConfiguration getCustomerConfiguration(@Nonnull IdentityData identityData,
        ...
        return getCustomerConfiguration(identityData, derivationOptions, null);
    }

    @Nonnull
    public CustomerConfiguration getCustomerConfiguration(@Nonnull IdentityData identityData,
                                                          @Nonnull ElementDerivationOptions derivationOptions,
                                                          @Nullable AuthoredElementValues editedValues) throws CommunicationException {
        var resolved = resolveSelected(identityData);
        return getCustomerConfigurationResolved(resolved, identityData, derivationOptions, editedValues);
    }
    ...
    private <C, I> CustomerConfiguration getCustomerConfigurationResolved(
            ...
            @Nonnull IdentityData identityData,
            @Nonnull ElementDerivationOptions derivationOptions,
            @Nullable AuthoredElementValues editedValues
    ) throws CommunicationException {
        var definition = (CommunicationProviderDefinition<C, I>) resolved.definition();
        var context = (CommunicationProviderContext<C, I>) resolved.context();
        ...
        if (layout == null) {
            return new CustomerConfiguration(null, DerivedRuntimeElementData.empty(), true, new AuthoredElementValues());
        }

        // Identity snapshots contain plain runtime data; only editable forms use authored envelopes.
        var authoredValues = editedValues != null ? editedValues
                : authoredInputValueService.toLiteralAuthoredElementValues(layout, identityData.communicationProviderData());
        ...
        return new CustomerConfiguration(layout, derived, ready, authoredValues);
    }
