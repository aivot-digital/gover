package de.aivot.gover.backend.utils.specification;
...
public record SpecificationBuilderJsonNotEquals<T>(
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        ...
        for (int i = 0; i < path.size(); i++) {
            args[i+1] = builder.literal(path.get(i));
        }

        var extractExpression = builder.function(JsonExtractPathFunctionName, Object.class, args);

        return builder
                .notEqual(extractExpression, value);
    }
}
