...
import de.aivot.gover.backend.form.entities.FormSlugHistoryEntity;
import de.aivot.gover.backend.form.enums.FormStatus;
import de.aivot.gover.backend.form.repositories.FormRepository;
import de.aivot.gover.backend.form.repositories.FormSlugHistoryRepository;
...
public class FormService implements EntityService<FormEntity, Integer> {
    ...
    private FormEntity cleanForm(@Nullable FormEntity prev, @Nonnull FormEntity updated) throws ResponseException {
            ...
            if (updatedDepartmentId == null) {
                ...
                if (previousIsSameAsUpdated) {
                    ...
                    if (departmentService.exists(updatedDepartmentId)) {
                        ...
                        throw ResponseException.badRequest("Die Organisationseinheit mit der ID " + updatedDepartmentId + " existiert nicht");
                    }
                }
            }
        };
        ...
        if (!notNull) {
            throw ResponseException.badRequest("Die entwickelnde Organisationseinheit ist erforderlich");
        }
