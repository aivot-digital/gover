...
describe('resolveComputedElementSubState', () => {
    ...
    it('should fall back to the index for legacy id-less sub states', () => {
        const wrappedLegacySubState = {
            id: null,
            states: {
                field: {
                    error: 'Wrapped legacy error',
                },
            },
        };
        const bareLegacySubState: ComputedElementStates = {
            field: {
                error: 'Bare legacy error',
            },
        };

        expect(resolveComputedElementSubState([wrappedLegacySubState], 'row-1', 0)).toBe(wrappedLegacySubState);
        expect(resolveComputedElementSubState([bareLegacySubState], 'row-1', 0)).toBe(bareLegacySubState);
    });
});
...
describe('applyComputedErrors', () => {
    ...
    it('should use computed sub states when derived data has no row structure', () => {
        const computedErrors: ComputedElementErrors = {
            list: {
                subStates: [
                    {
