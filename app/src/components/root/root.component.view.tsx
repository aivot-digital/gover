...
import {addError, clearCustomerInput, clearDisabled, clearErrors, selectLoadedForm} from '../../slices/app-slice';
...
export function RootComponentView({
    ...
    const isCurrentPageValid = async () => {
        ...
        if (currentStep === generalInformationStepIndex) {
            $debug.log(`Testing ${getElementNameForType(ElementType.IntroductionStep)}`);
            ...
            $debug.log(`Testing ${getElementNameForType(ElementType.SummaryStep)}`);
            ...
            $debug.log(`Testing ${getElementNameForType(ElementType.SubmitStep)}`);
        ...
        $debug.log(`${getElementNameForType(ElementType.SubmitStep)} is ${isValid ? '' : 'in'}valid`);
        $debug.end();
