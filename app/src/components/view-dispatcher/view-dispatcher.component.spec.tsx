...
import type {RichTextInputElement} from '../../models/elements/form/input/rich-text-input-element';
...
vi.mock('../../views', () => ({
    views: {
        [ElementType.Text]: (props: BaseViewProps<TextFieldElement, string>) => <input
            ...
            data-dynamic-text={props.inputModeLiteralContext?.dynamicText != null}
            value={props.value ?? ''}
            onChange={(event) => props.setValue(event.target.value)}
            onBlur={(event) => props.onBlur(event.target.value)}
        />,
        [ElementType.RichTextInput]: (props: BaseViewProps<RichTextInputElement, string>) => <textarea
            aria-label="Test value"
            data-dynamic-text={props.inputModeLiteralContext?.dynamicText != null}
        />,
        [ElementType.Number]: (props: BaseViewProps<NumberFieldElement, number>) => <input
            aria-label="Test value"
            data-dynamic-text={props.inputModeLiteralContext?.dynamicText != null}
        />,
    },
...
describe('ViewDispatcherComponent authored values', () => {
    it.each([
        [ElementType.Text, true, true],
        [ElementType.RichTextInput, true, true],
        [ElementType.Text, false, false],
        [ElementType.RichTextInput, false, false],
        [ElementType.Number, true, false],
    ] as const)('restricts dynamic text for type %s with input modes enabled: %s', (type, inputModesEnabled, expected) => {
        // Include the policy even on an unsupported type to cover untyped API input.
        const element = {
        ...
        expect(screen.getByRole('textbox', {name: 'Test value'}))
            .toHaveAttribute('data-dynamic-text', String(expected));
    });
