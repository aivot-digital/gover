...
class _LocalStorageService {
    ...
    public storeFlag(key: LocalstorageKey, value: boolean): void {
        if (value) {
            localStorage.setItem(_LocalStorageService.generateKey(key), 'yes');
        } else {
            this.clearItem(key);
        }
    }

    public loadFlag(key: LocalstorageKey): boolean {
