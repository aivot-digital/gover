...
import {clearCustomerInput, clearErrors, clearLoadedForm, redoLoadedForm, selectFutureLoadedForm, selectLoadedForm, selectPastLoadedForm, showDialog, undoLoadedForm, updateLoadedForm} from '../../../slices/app-slice';
...
export function FormEditPage(): JSX.Element {
    ...
    const futureLoadedForms = useAppSelector(selectFutureLoadedForm);
    const hasFutureLoadedForm = useMemo(() => futureLoadedForms.length > 0, [futureLoadedForms]);
    ...
    useEffect(() => {
        if (loadedForm?.themeId != null) {
            useThemesApi(api)
                .retrieveTheme(loadedForm.themeId)
                .then(setTheme)
                .catch((err) => {
                    console.error(err);
                });
    ...
    const handleUndo = () => {
        ...
        useFormsApi(api)
            .save(pastLoadedForm[pastLoadedForm.length - 1])
            .catch((err) => {
                console.error(err);
    ...
    const handleRedo = () => {
        ...
        useFormsApi(api)
            .save(futureLoadedForms[futureLoadedForms.length - 1])
            .catch((err) => {
                console.error(err);
                dispatch(showErrorSnackbar('Das Formular konnte nicht gespeichert werden.'));
            });
