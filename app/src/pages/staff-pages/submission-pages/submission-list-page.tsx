...
async function fetchSubmissions(api: Api, user: User, includeArchived: boolean | undefined, includeTest: boolean | undefined, onlyAssigned: boolean | undefined, formId: string | undefined): Promise<Submission[]> {
...
const columns: Array<GridColDef<Submission>> = [
    {
        ...
        renderCell: (params) => {
            ...
            return (
                ...
                >
                    <Tooltip title={determineLabel(params.row)}>
                        <Icon color={determineColor(params.row)} />
                    </Tooltip>
...
export function SubmissionListPage(): JSX.Element {
    ...
    useEffect(() => {
        if (user != null && api != null) {
            fetchSubmissions(api, user, includeArchived, includeTest, onlyAssigned, undefined)
                .then(setSubmissions)
                .catch((err) => {
                    console.error(err);
    ...
    }, [user, includeArchived, onlyAssigned, includeTest, api]);
...
function determineColor(sub: SubmissionListProjection): 'info' | 'error' | 'inherit' {
    if (sub.archived != null) {
        return 'inherit';
    } else if (sub.destinationId != null) {
        if (sub?.destinationSuccess ?? false) {
