...
async function fetchSubmissions(api: Api, user: User, includePaymentPending: boolean | undefined, includeArchived: boolean | undefined, includeTest: boolean | undefined, onlyAssigned: boolean | undefined): Promise<Submission[]> {
    const submissionsPage = await new SubmissionsApiService(api)
        .list(0, 999, 'created', 'DESC', {
            notPending: !includePaymentPending,
            notArchived: !includeArchived,
            notTestSubmission: !includeTest,
...
export function SubmissionListPage(): JSX.Element {
    ...
    useEffect(() => {
        if (user != null && api != null) {
            fetchSubmissions(api, user, includePaymentPending, includeArchived, includeTest, onlyAssigned)
                .then(setSubmissions)
                .catch((err) => {
                    console.error(err);
    ...
    return (
        <>
            ...
            >
                ...
                >
                    <ListItemIcon>
                        <SettingsSuggestOutlined color={includePaymentPending ? 'primary' : undefined} />
                    </ListItemIcon>
                    <ListItemText
                        id="switch-list-pending"
                        primary="Vorgänge in Verarbeitung durch das System anzeigen"
                    />
                    <Switch
                        edge="end"
