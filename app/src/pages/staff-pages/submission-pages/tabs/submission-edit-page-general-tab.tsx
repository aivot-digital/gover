...
function createGeneralRows(props: SubmissionEditPageGeneralTabProps) {
    const created = parseISO(props.submission.created);

    return [
        {
            icon: <DescriptionOutlinedIcon />,
            label: 'Formular',
            children: `${props.form.title} - Version ${props.form.version}`,
        },
        {
        ...
        {
            icon: (
                props.submission.reviewScore != null ?
                    <></> :
            ...
            children: (
                props.submission.reviewScore != null ?
                    <Rating
                        value={props.submission.reviewScore ?? undefined}
                    /> :
...
export function createAuthRows(props: SubmissionEditPageGeneralTabProps) {
...
export function SubmissionEditPageGeneralTab(props: SubmissionEditPageGeneralTabProps): JSX.Element {
    ...
    return (
        <>
            {
                Boolean(props.submission.isTestSubmission) &&
                <Box
                    sx={{mb: 4}}
                >
                    <AlertComponent
                        ...
                        <TableContainer>
                            ...
                            >
                                <TableBody>
                                    ...
                                    >
                                        {props.submission.fileNumber ?? 'Kein Aktenzeichen vergeben'}
                                    </StatusTableRow>
                                    ...
                                    >
                                        {props.submission.assigneeId != null ? userOptions?.find((opt) => opt.value === props.submission.assigneeId)?.label : 'Keine Mitarbeiter:in zugewiesen'}
                                    </StatusTableRow>
                                    ...
                                    >
                                        {format(archived, 'dd.MM.yyyy')} {format(archived, 'HH:mm')} Uhr
                                    </StatusTableRow>
                                </TableBody>
                            </Table>
                        </TableContainer>
                    ...
                    <TableContainer>
                        ...
                        >
                            <TableBody>
                                ...
                                >
                                    {destination?.name ?? 'Gelöschte Schnittstelle'}
                                </StatusTableRow>

                                <StatusTableRow
                                    label="Status"
                                    icon={
                                        props.submission.destinationSuccess ?
                                            <CheckCircleOutlineOutlinedIcon color="success" /> :
                                            <CancelOutlinedIcon color="error" />
                                    }
                                >
                                    Übertragung an Schnittstelle {props.submission.destinationSuccess ? 'erfolgreich' : 'fehlgeschlagen'} {
                                    (!props.submission.destinationSuccess && archived != null) ? '(Vorgang in Gover manuell abgeschlossen)' : ''
                                }
                                </StatusTableRow>

                                {
                                    props.submission.destinationResult != null &&
                                    isStringNotNullOrEmpty(props.submission.destinationResult) &&
                                    <StatusTableRow
                                        label={
                                            props.submission.destinationSuccess ?
                                                'Erfolgsmeldung' :
                                    ...
                                    >
                                        {props.submission.destinationResult}
                                    </StatusTableRow>
                                }

                                {
                                    ...
                                    <StatusTableRow
                                        ...
                                        icon={<AssignmentOutlinedIcon />}
                                    >
                                        {props.submission.fileNumber ?? 'Schnittstelle hat kein Aktenzeichen vergeben'}
                                    </StatusTableRow>
                                }
