...
describe('CustomerInstanceTaskView', () => {
    beforeEach(() => {
        ...
        vi.spyOn(ProcessInstanceTaskApiService.prototype, 'putCustomerTaskView')
            .mockResolvedValue(createTaskView({
                data: {field: literalAuthoredValue('saved')},
    ...
    it('reloads the task view when polling reports that the active task has finished', async () => {
        vi.mocked(CustomerTaskViewApiService.prototype.getTaskView)
            .mockResolvedValueOnce(createTaskView({data: {state: literalAuthoredValue('active')}}))
            .mockResolvedValueOnce(createTaskView({data: {state: literalAuthoredValue('completed')}, events: []}));
        const rendered = render(<CustomerInstanceTaskView/>);

        await waitFor(() => expect(mocks.elementDerivationProps?.authoredElementValues).toEqual({state: literalAuthoredValue('active')}));
        ...
        await waitFor(() => expect(mocks.elementDerivationProps?.authoredElementValues).toEqual({state: literalAuthoredValue('completed')}));
        expect(mocks.elementDerivationProps.readOnly).toBe(true);
        expect(mocks.elementDerivationProps.onEvent).toBeUndefined();
...
function createTaskView(overrides?: Partial<ReadyCustomerTaskView>): ReadyCustomerTaskView {
    return {
        ...
        data: {field: literalAuthoredValue('initial')},
