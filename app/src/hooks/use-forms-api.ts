...
interface FormsApi {
    ...
    listRevisions(id: number): Promise<Page<FormRevision>>;

    rollbackRevision(id: number, revisionId: number): Promise<Form>;
...
export function useFormsApi(api: Api): FormsApi {
    ...
    const listRevisions = async (id: number) => {
        return await api.get<Page<FormRevision>>(`forms/${id}/revisions`);
    };

    const rollbackRevision = async (id: number, revisionId: number) => {
        return await api.get<Form>(`forms/${id}/revisions/rollback/${revisionId}`);
    };
    ...
    const rateApplication = async (submission: SubmissionListProjection, score: number) => {
        return await api.getPublic<string>(
            `rate/${submission.id}`, {queryParams: {score: score.toFixed(0)}},
        );
    };
