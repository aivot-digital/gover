...
public interface VUserDepartmentPermissionRepository extends JpaRepository<VUserDepartmentPermissionEntity, VUserDepartmentPermissionEntityId>, JpaSpecificationExecutor<VUserDepartmentPermissionEntity> {
    @Query(
            "SELECT EXISTS(SELECT 1 FROM v_user_department_permissions p WHERE p.user_id = :userId AND p.department_id = :departmentId AND p.permissions @> ARRAY[:permission])"
    )
    ...
    @Query(
            "SELECT p.department_id FROM v_user_department_permissions p WHERE p.user_id = :userId AND p.permissions @> ARRAY[:permission]"
    )
