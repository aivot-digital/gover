...
public class TableInputElement extends BaseInputElement<List<Map<String, Object>>> implements PrintableElement<List<Map<String, Object>>> {
    @Nullable
    private List<TableInputElementColumn> fields;
    ...
    public void performValidation(@Nullable List<Map<String, Object>> value) throws ValidationException {
        ...
        for (Map<String, Object> row : value) {
            for (TableInputElementColumn col : fields) {
                ...
                if (TableColumnDataType.Number == col.getDatatype()) {
                    if (val == null) {
                        ...
                        if (dValue.compareTo(NumberInputElement.AbsoluteMinValue) < 0) {
                            var msg = String.format(
                                    "Der Wert in Spalte \"%s\" in Zeile %d muss mindestens %s betragen.",
                                    col.getLabel(),
                                    rowNumber,
                                    NumberInputElement.formatGermanNumber(NumberInputElement.AbsoluteMinValue, 0)
                            );
                            throw new ValidationException(this, msg);
                        }

                        if (dValue.compareTo(NumberInputElement.AbsoluteMaxValue) > 0) {
                            var msg = String.format(
                                    "Der Wert in Spalte %s in Zeile %d darf maximal %s betragen.",
                                    col.getLabel(),
                                    rowNumber,
                                    NumberInputElement.formatGermanNumber(NumberInputElement.AbsoluteMaxValue, 0)
                            );
                            throw new ValidationException(this, msg);
                        }
                    } else {
    ...
    public boolean equals(Object o) {
        ...
        TableInputElement that = (TableInputElement) o;
    ...
    public List<TableInputElementColumn> getFields() {
        return fields;
    }

    public TableInputElement setFields(List<TableInputElementColumn> fields) {
        this.fields = fields;
        return this;
    }
    ...
    public TableInputElement setMaximumRows(Integer maximumRows) {
    ...
    public TableInputElement setMinimumRequiredRows(Integer minimumRequiredRows) {
