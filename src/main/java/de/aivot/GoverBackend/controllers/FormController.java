...
public class FormController {
    ...
    public FormPublicProjection retrievePublic(
        ...
        return formRepository
                .getLatestBySlugAndStatus(slug, ApplicationStatus.Published)
                .orElseThrow(NotFoundException::new);
    }
    ...
    @GetMapping("/api/forms/{id}/revisions")
    public Collection<FormRevision> listRevisions(
            @AuthenticationPrincipal Jwt jwt,
            @PathVariable Integer id
    ) {
        var user = KeyCloakDetailsUser.fromJwt(jwt);
        ...
        return formRevisionRepository
                .getAllByFormIdOrderByTimestampDesc(app.getId(), Limit.of(20));
    }
